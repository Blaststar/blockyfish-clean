...
var logsDirectoryPath = path.join(app.getPath("userData"), "client.log");
var logsDirectory = path.join(app.getPath("userData"));

try 
{
	fs.writeFileSync(logsDirectoryPath, "");
} catch (err) 
{
	console.error(err);
}
...
{
    fs.appendFile(logsDirectoryPath, message + "\n", (err) => 
    {
...
if (fs.existsSync(downloadPath + "/blockyfishclient-update-download.exe")) {
	fs.unlink(downloadPath + "/blockyfishclient-update-download.exe", (err) => {
		if (err) {
			...
			addLog(`[UPDATER] ${e.message}`);
			return;
		}
		addLog(`[UPDATER] Successfully remove file`);
	});
}
...
app.whenReady().then(async function makeNewWindow() {
	const createWindow = () => {
		...
		if (!extensionsLoaded) {
            ...
            docassetsPath = app.getAppPath() + `/extensions/docassets`;
		...
		function makeNewWindow() {
			...
			win.webContents.on("did-fail-load", (_event, _errorCode, errorDescription) => {
				if (errorDescription == "ERR_INTERNET_DISCONNECTED") {
					...
					win.show();
					setTimeout(() => {
						win.loadURL("https://beta.deeeep.io").then((site) =>
                        {
                            site.title = "Blockyfish Clean";
                        });
					}, 5000);
				}
			});
            ...
            `);
                ...
                pluginFolderOpen.addEventListener("click", () => {
                    console.log("PLUGIN_FOLDER_OPEN")
                ...
                const logFolderOpen = document.querySelector(".logs-file-open")
                logFolderOpen.addEventListener("click", () => {
                    console.log("LOG_FILE_OPEN")
                    document.body.style.cursor = "progress"
                    pluginFolderOpen.style.cursor = "progress"
                    setTimeout(() => {
                        document.body.style.cursor = ""
				...
				fs.readdir(pluginDirectoryPath, function (err, files) {
					if (err) {
						return addLog("[ERROR] Unable to scan directory: " + err);
                ...
                setTimeout(() => {
                    ...
                    async function getBlockyfishNews() {
                        ...
                        let cleanNews = "<b>Clean edition</b><p>Same old blocky you know and love, just cleaned up a little by Blaststar :)</p><br /><hr /><br />";
                        blockyfish_news.innerHTML = cleanNews + news;
                    }
                    getBlockyfishNews()
                ...
                discord.addEventListener("click", () => {
                    window.open('https://discord.gg/h2kbkfZEFk')
                })
					...
					if (!global.consoleLogScriptRunning) {
						win.webContents.on("console-message", (ev, level, message, line, file) => {
							...
							if (matches(msg, "ERR_INTERNET_DISCONNECTED")) {
								addLog("[CONNECTION_ERROR] Internet connection required")
							}
							...
							if (matches(msg, "PLUGIN_FOLDER_OPEN")) {
								shell.openPath(path.join(app.getPath("userData"), "plugins"));
							}

                            if (matches(msg, "LOG_FILE_OPEN")) {
								shell.openPath(logsDirectory).then(() =>
                                {
                                    shell.openPath(logsDirectoryPath);
                                });
							}
            ...
            `);
					...
					function setGameMode(mode, menu) {
						try {
							...
							addLog("[ERROR] Unable to set game mode");
						}
						...
						if (matches(currentUrl, "/u/")) {
							...
							request("https://apibeta.deeeep.io/users/u/" + currentUrl.replace("https://beta.deeeep.io/u/", "").replace(/\?host=....../i, ""), { json: true }, (error, res, body) => {
								if (error) {
									return addLog("[ERROR] " + error.message);
								}
								if (!error && res.statusCode == 200) {
